...
//  trucs qui ressemblent à tout sauf des haïkus. série numéro +l'infini
//
//
//Homonymes :
//
                                ...
                                int testValue = 0)
{
    ...
    if (testValue) {
        if ((currentNodeValue_i < minValue) || (currentNodeValue_i > maxValue)) {
            errorMessage = "la valeur indiquée est trop grande ou trop petite";
            return 0;
        }
    }
    *numericValue = currentNodeValue_i;
    if (!ReadNextNode()) { return 0; }
    if (!GotAnEndBaliz(balizName)) {
        errorMessage = "erreur dans la balise " + balizName;
        return 0;
...
int XMLParser::ReadNodeOfColor(string balizName, Uint8 *_red, Uint8 *_green, Uint8 *_blue)
{
    int stop = 0;
    Sint32 red = -1;
    Sint32 green = -1;
    ...
    while (!stop) {
        if (GotAStartBaliz("Red")) {
            if (!ReadNumericValue("Red", &red, 0, 255, 1))
                { return 0; }
        } else if (GotAStartBaliz("Blue")) {
            if (!ReadNumericValue("Blue", &blue, 0, 255, 1))
                { return 0; }
        } else if (GotAStartBaliz("Green")) {
            if (!ReadNumericValue("Green", &green, 0, 255, 1))
                { return 0; }
        } else {
            stop = 1;
        }
    ...
    if ((red == -1) || (blue == -1) || (green == -1)) {
        errorMessage = "Toutes les couleurs n'ont pas été définies!";
        return 0;
    }
